...
#undef BIAS_FUNC
		...
		for (uint i = 0; i < directional_lights_count; i++) {
			...
			vec3 light_color = directional_lights.data[i].color;
				...
				// Directional light projector (separate texture binding, hardware repeat wrapping).
				if (sc_use_directional_projector() && directional_lights.data[i].has_projector != 0u) {
				...
				splane /= splane.w;
					...
					splane_ddx /= splane_ddx.w;
						vec2 proj_uv_ddx = (splane_ddx.xy - splane.xy) * 0.5;

					vec4 splane_ddy = (directional_lights.data[i].projector_matrix * vec4(vertex + vertex_ddy, 1.0));
					splane_ddy /= splane_ddy.w;
						vec2 proj_uv_ddy = (splane_ddy.xy - splane.xy) * 0.5;

						vec2 proj_uv = (splane.xy * 0.5 + 0.5) + directional_lights.data[i].projector_offset.xy;

						vec4 proj = textureGrad(sampler2D(directional_projector_texture, SAMPLER_LINEAR_WITH_MIPMAPS_REPEAT), proj_uv, proj_uv_ddx, proj_uv_ddy);
					light_color *= proj.rgb * proj.a;
				} else {
						vec2 proj_uv = (splane.xy * 0.5 + 0.5) + directional_lights.data[i].projector_offset.xy;

						vec4 proj = textureLod(sampler2D(directional_projector_texture, SAMPLER_LINEAR_REPEAT), proj_uv, 0.0);
					light_color *= proj.rgb * proj.a;
				}
			}
